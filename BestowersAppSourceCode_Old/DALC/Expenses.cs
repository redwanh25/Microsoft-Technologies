...
namespace CIS.Lib.DALC
{
    public class Expenses : DAOBase
    {
        ...
        public bool Load(int id)
        {
            ...
            _expenseCategoryId = Convert.ToInt32(dr["ExpenseCategoryID"]);
        ...
        public DataSet Find(string whereClause, string orderBy, string strLimit)
        {
            ...
            string cmdText = @"SELECT " + strLimit + @" e.*, ec.Name as CategoryName,
                                f.Name as FundName, 'datacell' as TRClass      
						        FROM Expenses e, ExpenseCategory ec, Fund f
                                WHERE e.ExpenseCategoryID = ec.ID 
                                    AND e.FundID = f.ID 
                            ";
        ...
        public int Insert(int expenseCategoryId, double amount, int fundId, DateTime expenseDate, string purpose, string comments, int treeId, int organizationId)
        {
            string cmdText = @"INSERT INTO Expenses (ExpenseCategoryID, Amount, FundID, ExpenseDate, Purpose, Comments, TreeID, OrganizationID) 
							VALUES (@0, @1, @2, @3, @4, @5, @6, @7)";
            int rowsAffected = SqlHelper.ExecuteNonQuery(AppSetting.GetConnString(), cmdText, expenseCategoryId, amount, fundId, expenseDate, purpose, comments, treeId, organizationId);

            return rowsAffected;
        }

        public int Update(int id, int expenseCategoryId, double amount, int fundId, DateTime expenseDate, string purpose, string comments, int treeId, int organizationId)
        {
            string cmdText = @"UPDATE Expenses SET ExpenseCategoryID=@0, Amount=@1, FundID=@2, ExpenseDate=@3, Purpose=@4, Comments=@5, TreeID=@6, OrganizationID = @7 WHERE ID=@8";
            int rowsAffected = SqlHelper.ExecuteNonQuery(AppSetting.GetConnString(), cmdText, expenseCategoryId, amount, fundId, expenseDate, purpose, comments, treeId, organizationId, id);
            return rowsAffected;
        }
