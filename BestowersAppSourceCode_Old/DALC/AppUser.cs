...
namespace CIS.Lib.DALC
{
	...
	public class AppUser : DAOBase
	{
        ...
        public int Insert(int roleId, string firstName, string lastName, string email, string userName, string passwd)
        {
            string cmdText = @"INSERT INTO AppUser (RoleId,FirstName,LastName,Email,UserName,Password) 
							VALUES (@0,@1,@2,@3,@4,@5)";
            int rowsAffected = SqlHelper.ExecuteNonQuery(AppSetting.GetConnString(), cmdText, roleId, firstName, lastName, email, userName, passwd);

            return rowsAffected;
        }

        public int Update(int userId, int roleId, string firstName, string lastName, string email, string userName, string passwd)
		{
			string cmdText = @"UPDATE AppUser SET RoleID=@0, UserName=@1,Password=@2,Email=@3,FirstName=@4,LastName=@5 WHERE UserID=@6";
            int rowsAffected = SqlHelper.ExecuteNonQuery(AppSetting.GetConnString(), cmdText, roleId, userName, passwd, email, firstName, lastName, userId); 
			return rowsAffected;			
		}
